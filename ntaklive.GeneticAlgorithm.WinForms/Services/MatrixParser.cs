...
public class MatrixParser : IMatrixParser
{
    ...
    // private static IReadOnlyList<T> ParseRow<T>(IXLWorksheet worksheet, int length, int columnOffset, int rowOffset)
    // {
    //     IXLRange row = worksheet.Range(rowOffset, columnOffset, rowOffset + length, columnOffset + length);
    //
    //     var values = new List<T>();
    //
    //     for (int i = 0; i < row.; i++)
